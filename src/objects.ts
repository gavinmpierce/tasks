...
export function isValid(question: Question, answer: string): boolean {
    ...
    const checkAnswer = question.options.includes(answer);
    return checkAnswer;
}
...
export function toShortForm(question: Question): string {
    const shortForm = `${question.id}: ${question.name.substring(0, 10)}`;
    return shortForm;
}
...
export function toMarkdown(question: Question): string {
    if (question.type !== "multiple_choice_question") {
        ...
        return formatedShortQ;
    }
    let formatedMultipleQ = `# ${question.name}\n${question.body}`;
    for (const option of question.options) {
        formatedMultipleQ += `\n- ${option}`;
    }
    return formatedMultipleQ;
}
